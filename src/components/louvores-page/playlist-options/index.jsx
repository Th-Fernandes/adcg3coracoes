...
export function PlaylistOptionsLouvoresPage() {
  ...
  function handleRedirectToPlatform(platform) {
    platform.includes('Youtube')
      ? (
        nextRouter.push('https://www.youtube.com/watch?v=5QHF5OQeFOs&ab_channel=CASAWorship')
      )
      : (
        nextRouter.push('https://open.spotify.com/playlist/70pcCrwZu8fI3K9MyhCP3T?utm_source=generator&theme=0')
      )
          ...
          platforms.map(({ title, img }, index ) => (
            <li className={louvoresPageStyle["playlist-option"]} key={index}>
              <article>
                ...
                <div
                  onClick={() => handleRedirectToPlatform(title) }
